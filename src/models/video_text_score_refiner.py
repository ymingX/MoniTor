...
class VideoTextScoreRefiner:
    def __init__(
        ...
        self.output_indices_dir = Path(output_indices_dir)
        ...
        batch_clip_frame_paths = [
            [
                ...
                for frame_idx in range(
                    ...
                    min(clip_center_frame + 1 , video.num_frames),
                )
            ]
    ...
    def _retrieve_captions(
        ...
        for idx, frame_idx in enumerate(batch_frame_idxs):
            ...
            for nn_idx in range(min(max(1,self.num_neighbors),batch_start_frame+1)):
                #print(f"indices length: {len(indices)}, file_names length: {len(file_names)}")
                #print(f"indices[{idx}] length: {len(indices[idx])}, indices[{idx}][{nn_idx}] value: {indices[idx][nn_idx]}")

                file_name = file_names[indices[idx][nn_idx]]
                nn_file_names[str(nn_idx)] = file_name
                similarity = distances[idx][nn_idx]
                pos = indices[idx][nn_idx]
                pos = np.array(pos)
                ret_index = file_name.split("/")[-1]
                frame_captions[str(nn_idx)] = video_captions[ret_index]
                frame_similarity[str(nn_idx)] = similarity.item()
                frame_pos[str(nn_idx)] = pos.item()
            ...
            video_pos_nn[str(frame_idx)] = frame_pos
            
    #def _save_results(self, video_name, video_captions_nn, video_similarity_nn, ret_file_names_nn):
    def _save_results(self, video_name, video_captions_nn, video_similarity_nn, video_pos_nn, ret_file_names_nn):
    ...
    def retrieve_nn(self, video: VideoRecord):
        ...
        for batch_start_frame in tqdm(
            ...
            # Create a sub-index using the first 'cnt' elements
                index_tmp = faiss.IndexFlatL2(index.d)  # Assuming 'index' is IndexFlatL2, you may need to adapt to your index type
                index_tmp.add(index.reconstruct_n(0, min(cnt,index.ntotal)))  # Add vectors from the original index to the temporary sub-index
    
                distances_i, indices_i = index_tmp.search(np.array([vector]),  min(max(1,self.num_neighbors),batch_start_frame+1))
                distances.append(distances_i[0])
                
    
                indices.append(indices_i[0].tolist())
                cnt += 1
            
            def get_list_dimensions(lst):
                if isinstance(lst, list):
                    return [len(lst)] + get_list_dimensions(lst[0]) if len(lst) > 0 else []
                else:
                    return []

            dimensions = get_list_dimensions(indices)
        ...
        self._save_results(video_name, video_captions_nn, video_similarity_nn, video_pos_nn, ret_file_names_nn)
...
def parse_args():
    ...
    parser.add_argument("--output_indices_dir", type=str, required=True)
