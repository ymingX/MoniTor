#You can replace any prompt with a Chinese version for better performance.
import argparse
import json
import re
import time
...
class GLMAnomalyScorer:
    def __init__(
        ...
        self.prediction1 = "No prediction available" 
        self.prediction2 = "No prediction available"  
    ...
    def _lstm_summarize(self, texts):
        ...
        summary = self.client.chat.completions.create(
            model="glm-4-flash",
            messages=[
                {"role": "system", "content": "summary_prompt."}, # modify to your summary prompt
        ...
        for batch_start in tqdm(range(0, len(frame_idxs), self.batch_size), desc=f"Processing {video.path}", unit="batch"):
            ...
            for result, frame_idx in zip(results, batch_frame_idxs):
                ...
                if previous_score is not None and previous_score != -1:
                    score = round(0.6 * current_score + 0.4 * previous_score, 1)
                else:
